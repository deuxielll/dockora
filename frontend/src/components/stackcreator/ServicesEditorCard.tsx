...
const ServicesEditorCard = ({
  ...
  return (
    <div className={`p-6 rounded-xl ${panelClasses} flex-1 flex flex-col ${className}`}>
      <fieldset disabled={disabled} className="flex-1 flex flex-col overflow-y-auto no-scrollbar space-y-6"> {/* Added space-y-6 here */}
        {editorMode === 'visual' ? (
          <> {/* Using a React Fragment to group elements without an extra div */}
            ...
            <button onClick={addService} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-all duration-300 focus:outline-none bg-dark-bg text-gray-300 shadow-neo active:shadow-neo-inset">
